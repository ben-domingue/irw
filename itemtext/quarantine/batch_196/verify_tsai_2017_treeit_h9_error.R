# verify_tsai_2017_treeit_h9_error.R -- Step 5b mapping check (batch_196).
# Built from references/verify_template.R.
#
# CLAIM: live item codes H9-1..H9-4 are the S3 File (.xlsx) column names (data/tsai_2017_treeit.py
# melts by column name, no rename, resp kept 1-5), and H9-k is the k-th statement printed under
# "H9. Error" in the study's S1 File (Appendix 1, Treeit Heuristic Evaluation, pone.0180102.s001;
# header and description line at the foot of p.5, column header and the four statements at the
# top of p.6):
#   H9-1 "Interfaces that make errors impossible."
#   H9-2 "The system provides avoid modes or uses informative feedback, e.g., different sounds."
#   H9-3 "Execution error vs. evaluation error."
#   H9-4 "The system can prevent various types of slips and mistakes."
...
#      each heuristic as the mean of its Hk-* columns must reproduce them. H9 sits in System
#      Support (H7, H8, H9, H11, H12): H9 item-total r 0.768, alpha 0.855, mean 4.031.
#   A2. Controls, scored against the WHOLE System Support profile (5 item-total r + alpha + mean),
#      not H9's own r alone: shifting the H9 window one column left [H8-3,H9-1..H9-3] or right
#      [H9-2..H9-4,H10-1], dropping any one H9 column, widening by one column either side,
#      substituting any other heuristic's columns in the H9 slot, and replacing any single H9
#      column with any foreign H-column must NOT reproduce it. The nearest control is printed.
#   B. Block size: S1 prints 4 statements under H9; the xlsx has 4 H9 columns.
#   C. Live table == the xlsx H9-* columns (per-item response-level counts 1..5).
#   D. (not scored) within-block and cross-heuristic identical-answer counts and correlations.
#
# NOT ESTABLISHED: the order of the four statements within H9. A heuristic mean (and therefore
# every Table 3/4 statistic) is invariant to permuting its columns, no per-item statistic is
# published, and all four items share range and polarity, so all 24 within-block permutations
# pass Route A. Hence PARTIAL, not VERIFIED.

suppressMessages(library(irw))
TABLE <- "tsai_2017_treeit_h9_error"
SI <- "https://journals.plos.org/plosone/article/file?type=supplementary&id=10.1371/journal.pone.0180102.s003"
CACHE <- "itemtext/.cache/tsai_2017_treeit_h9_error/s003.xlsx"  # fallback if offline
H9 <- paste0("H9-", 1:4)
TOL <- 0.0015
...
names(dat) <- hdr
dat <- dat[!is.na(dat[["H9-1"]]), , drop = FALSE]
num <- function(cc) sapply(dat[cc], function(v) as.numeric(v))
ok <- TRUE
cat(sprintf("respondent rows: %d\n", nrow(dat)))
...
cat(sprintf("H9: S1 %d, xlsx %d (blocks agreeing overall: %d/14; H8 is the known 4-vs-3 mismatch)\n",
            s1_counts["H9"], xl_counts["H9"], sum(s1_counts == xl_counts)))
if (s1_counts["H9"] != 4 || xl_counts["H9"] != 4 || !identical(cols$H9, H9)) ok <- FALSE
...
# ---- Route A2: controls in the H9 slot, scored against the WHOLE System Support profile.
SSH <- pub$SS$h
SSPUB <- c(pub$SS$itc, pub$SS$a, pub$SS$mean)
ss_with <- function(h9vec) {
  MM <- M[, SSH]; MM[, "H9"] <- h9vec
...
wins <- list(published = hdr[j:(j + 3)], shift_left = hdr[(j - 1):(j + 2)], shift_right = hdr[(j + 1):(j + 4)],
             drop_H9_1 = H9[-1], drop_H9_2 = H9[-2], drop_H9_3 = H9[-3], drop_H9_4 = H9[-4],
             widen_left = hdr[(j - 1):(j + 3)], widen_right = hdr[j:(j + 4)])
cat("\nRoute A2 -- H9 slot controls vs published System Support profile:",
    paste(sprintf("%s %.3f", c(SSH, "alpha", "mean"), SSPUB), collapse = " "), "\n")
ctl_best <- Inf; ctl_lab <- ""
for (w in names(wins)) {
  ...
  if (w != "published") {
    if (hits(v)) ok <- FALSE
    if (max(abs(v - SSPUB)) < ctl_best) { ctl_best <- max(abs(v - SSPUB)); ctl_lab <- w }
  }
}
cat("other heuristic's columns in the H9 slot:\n")
for (h in setdiff(names(cols), "H9")) {
  v <- ss_with(M[, h]); show(h, h, v); if (hits(v)) ok <- FALSE
  if (max(abs(v - SSPUB)) < ctl_best) { ctl_best <- max(abs(v - SSPUB)); ctl_lab <- paste0("block ", h) }
}
allH <- grep("^H[0-9]+-[0-9]+$", hdr, value = TRUE)
und <- character(0); best <- Inf; bestlab <- ""; bestv <- NULL
for (o in setdiff(allH, H9)) for (k in 1:4) {
  w <- H9; w[k] <- o
  v <- ss_with(rowMeans(num(w))); dv <- max(abs(v - SSPUB))
  if (dv < best) { best <- dv; bestlab <- sprintf("%s<-%s", H9[k], o); bestv <- v }
  if (hits(v)) und <- c(und, sprintf("%s<-%s", H9[k], o))
}
cat(sprintf("single foreign-column replacements tried: %d; nearest %s max|dev| %.4f; reproducing: %s\n",
            4 * length(setdiff(allH, H9)), bestlab, best, if (length(und)) paste(und, collapse = ", ") else "none"))
show("nearest", bestlab, bestv)
if (best < ctl_best) { ctl_best <- best; ctl_lab <- bestlab }
cat(sprintf("NEAREST CONTROL overall: %s at max|dev| %.4f (tol %.4f)\n", ctl_lab, ctl_best, TOL))
if (length(und)) ok <- FALSE

# ---- Route C: live table equals the xlsx H9 columns (404-row table)
d <- irw::irw_fetch(TABLE)
cat(sprintf("\nRoute C -- response-level counts 1..5 per item, xlsx vs live (live rows %d)\n", nrow(d)))
for (it in H9) {
...
# ---- Route D (not scored)
X9 <- num(H9)
cat("\nWithin-H9 identical answers (of", nrow(dat), ") / r  [not scored]:\n")
for (a in 1:3) for (b in (a + 1):4)
  cat(sprintf("  %s~%s %3d  r %.3f\n", H9[a], H9[b], sum(X9[, a] == X9[, b]), cor(X9[, a], X9[, b])))
A <- num(allH)
cat("Most-identical columns outside H9 [not scored]:\n")
for (it in H9) {
  s <- sapply(setdiff(allH, H9), function(o) sum(A[, it] == A[, o])); o <- order(-s)[1:3]
  cat(sprintf("  %s: %s | r with top: %.3f\n", it, paste(sprintf("%s %d", names(s)[o], s[o]), collapse = "; "),
              cor(A[, it], A[, names(s)[o[1]]])))
}
cat("NOT ESTABLISHED: order of the four statements within H9 -- heuristic means are permutation-\n",
    "invariant and no per-item statistics are published; the identical-answer pattern above is\n",
    "circumstantial and is not scored.\n", sep = "")
