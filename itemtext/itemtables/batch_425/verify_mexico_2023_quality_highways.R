# verify_mexico_2023_quality_highways.R -- batch_425, route 9 (response-frequency matching), per wave.
#
# Claim: IRW item p5_13_k is INEGI ENCIG 2023 microdata column P5_13_K, printed in the 2023
# questionnaire (p.11) as question 5.13 option K (toll highways, "autopistas con casetas de
# cuota"), and p5_13a is P5_13A (question 5.13a, satisfaction). Since the #2415 rebuild
# (data/mexico_2023_quality.py) the 2021 rows (cov_year = 2021) come from ENCIG 2021 column
# P5_11_K / P5_11A -- 2021 question 5.11 (p.10), the same toll-highway question under INEGI's
# earlier numbering -- renamed onto the 2023 name (RENAME_2021) before stacking. 2021's own
# 5.13 does not exist (its section V ends at 5.11).
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P5_13A kept 1..6.
#
...
# permutation of codes -- or a mis-mapped 2021 rename onto trains (2021 5.10) -- fails.
# Corroboration: every 5.13 (2023) / 5.11 (2021) answerer reported toll-highway use in 5.1
# (2023 option 12 P5_1_12, 2021 option 10 P5_1_10).
#
...
C <- ".cache/mexico_2023_quality_highways"
...
items <- c(paste0("p5_13_", 1:5), "p5_13a")
recode <- function(m, v) {
...
map23 <- setNames(c(paste0("P5_13_", 1:5), "P5_13A"), items)
map21 <- setNames(c(paste0("P5_11_", 1:5), "P5_11A"), items)     # #2415 harmonised mapping
...
# Block corroboration: the 5.1 filter question (toll-highway user)
f23u <- w23[!is.na(recode(w23, "P5_13_1")) | !is.na(recode(w23, "P5_13A"))]
f21u <- w21[!is.na(recode(w21, "P5_11_1")) | !is.na(recode(w21, "P5_11A"))]
cat(sprintf("\n2023 5.13 answerers: %d; of these P5_1_12 == 1: %d\n", nrow(f23u), sum(f23u$P5_1_12 == "1")))
cat(sprintf("2021 5.11 answerers: %d; of these P5_1_10 == 1: %d\n", nrow(f21u), sum(f21u$P5_1_10 == "1")))
ok_all <- ok_all && all(f23u$P5_1_12 == "1") && all(f21u$P5_1_10 == "1")

cat("Not established by this route: that column P5_13_K (2023) / P5_11_K (2021) is questionnaire option K --\n",
