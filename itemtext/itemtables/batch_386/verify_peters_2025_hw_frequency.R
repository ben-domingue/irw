# Step 5b verification for peters_2025_hw_frequency (batch_386).
#
# CLAIM UNDER TEST
#   The 7 IRW items are the 7 checkbox options of the Your COVID-19 Risk tool's
#   LimeSurvey multiple-choice question `hwFrequency`. data/peters_2025_covid19_risk_precautions.py
#   takes each raw column "hwFrequency.<code>." and lower-cases <code> (snzCf -> snzcf),
...
#   resp 1 = option ticked ("Y"), resp 0 = not ticked.
#
# ROUTES
#   A. Label match, two independent copies: shipped item_text vs the REQs sheet
...
#      100110 exports (Est-not-null gate; "Y" -> 1, blank -> 0), merged to
#      irw_fetch() on (id, item). A swap of any two codes moves a response vector
#      onto the wrong code and breaks the merge, so every item is distinguished.
#   C. Keying polarity (route 6): `never` ("I never or only occasionally wash/clean
#      my hands") must correlate negatively with all six situation items, and the
#      six must correlate positively with one another.
#
# WHAT IT DOES NOT ESTABLISH
#   - The raw-column -> wording tie is a label match (ls_answer_code = subquestion
#     title = raw column infix); response data cannot test wording for the six
#     situation items, which C does not separate from one another.
#   - Only one deployment (sid 100110, overwhelmingly English) is re-run; the live
#     table pools 21 sids.
#   - resp 0 conflates "not ticked" with "question skipped" (the question was not
#     mandatory); nothing here can separate those.

suppressMessages(library(irw))

TABLE <- "peters_2025_hw_frequency"
SID   <- "100110"
...
qrow <- which(cls == "Q" & nm == "hwFrequency" & lg == "en")
...
cat(sprintf("   codes in sheet %d, in deployed survey %d, shipped %d; text identical in both: %d/7; stem identical: %s\n\n",
            length(sheet), length(sq), nrow(st), a_ok, stem_ok))
...
long <- do.call(rbind, lapply(FILES, function(fn) {
  ...
  names(df)[1] <- "orig_id"
  cols <- grep("^hwFrequency\\.[A-Za-z]+\\.$", names(df), value = TRUE)
  do.call(rbind, lapply(cols, function(cn) {
    code <- sub("^hwFrequency\\.([A-Za-z]+)\\.$", "\\1", cn)
    est  <- paste0("hwFrequency", toupper(substr(code, 1, 1)), substring(code, 2), "Est")
...
b_ok <- nrow(long) > 0 && unmatched == 0 && mismatch == 0 && length(unique(long$item)) == 7

# ---- C. polarity of `never` over the full live table ----------------------
w <- stats::reshape(d[, c("id", "item", "resp")], idvar = "id", timevar = "item", direction = "wide")
names(w) <- sub("^resp\\.", "", names(w))
cm <- stats::cor(w[, setdiff(names(w), "id")])
six <- setdiff(colnames(cm), "never")
nev <- cm["never", six]
pairs <- cm[six, six][upper.tri(cm[six, six])]
cat(sprintf("\nC. cor(never, other) over %d respondents: %s\n", nrow(w),
            paste(sprintf("%s %.2f", six, nev), collapse = ", ")))
cat(sprintf("   15 situation-item pairs: min %.2f, max %.2f\n", min(pairs), max(pairs)))
c_ok <- all(nev < 0) && all(pairs > 0)

ok <- a_ok == 7 && stem_ok && b_ok && c_ok
cat(sprintf("\nA %s  B %s  C %s\n", a_ok == 7 && stem_ok, b_ok, c_ok))
cat("Not established: wording for the six situation items rests on the code label match\n",
    "(A); C separates only `never` from the rest; one of 21 deployments re-run in B.\n", sep = "")
