# verify_mexico_2023_quality_trash.R -- batch_428, route 9 (response-frequency matching), per wave.
#
# Claim: IRW item p4_5_k is INEGI ENCIG microdata column P4_5_K, printed in both the 2023 and the
# 2021 questionnaire (Seccion IV, p.5 of each PDF) as question 4.5 option K (garbage collection,
# "la recoleccion de basura de esta ciudad"), and p4_5a is P4_5A (question 4.5a, satisfaction).
# Section IV was NOT renumbered between waves: RENAME_2021 in data/mexico_2023_quality.py has no
# p4_* entry and ASKED_2021 lists p4_5_1..3, p4_5a, so 2021 rows (cov_year = 2021) come from the
# same-named 2021 columns.
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P4_5A kept 1..6.
#
...
# permutation of the three yes/no codes, a flipped Si/No direction, or a mix-up with a
# neighbouring service block (4.4 parks, 4.6 police, 4.7 streets and avenues, ...) fails. P4_5_2_ESP / P4_5_2_TXT (the
# weekly-amount follow-up) are excluded from the pool by the regex, as are the 0-10 grades *B.
#
...
C <- ".cache/mexico_2023_quality_trash"
...
items <- c(paste0("p4_5_", 1:3), "p4_5a")
recode <- function(m, v) {
...
map23 <- setNames(c(paste0("P4_5_", 1:3), "P4_5A"), items)
...
cat("\nNot established by this route: that column P4_5_K is questionnaire option K of question 4.5 --\n",
