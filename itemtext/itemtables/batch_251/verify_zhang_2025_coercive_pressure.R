# verify_zhang_2025_coercive_pressure.R -- Step 5b mapping check (batch_251).
#
# Claim: live item CP<k> is the item the paper's S2 File appendix prints as
# "CP<k>: <wording>". The appendix prefixes each item with the very code the S1
# Data column carries (explicit code labels), and data/zhang_2025_green_supply_chain.py
# melts S1 columns CP1..CP4 by name, so the code IS the source column name.
#
# Falsifiable checks run here:
#   A. live CP<k> equals S1 Data column CP<k> id-for-id, and no other CP column
#      reaches full agreement (the live code -> S1 column tie);
#   B. 5-factor CFA on S1 (CP, NP, EA, SE, GSCII, as in the paper) reproduces the
#      paper's Table 2 standardized loadings keyed to CP1..CP4 -- 0.759/0.662/
#      0.810/0.852 -- and every non-identity permutation of the four codes fits worse;
#   C. Cronbach's alpha of the live CP items vs Table 2 alpha 0.853.
# NOT established: that the S2 appendix's "CP<k>" labels and Table 2's "CP<k>"
...
PUB_LOAD <- c(CP1 = 0.759, CP2 = 0.662, CP3 = 0.810, CP4 = 0.852)   # Table 2 (t002)
PUB_ALPHA <- 0.853
TOL <- 0.02
...
if (ok) {
  ...
  obs <- setNames(ss$est.std[ss$lhs == "CP" & ss$op == "=~"], ss$rhs[ss$lhs == "CP" & ss$op == "=~"])[items]
} else {
  cat("S1 download failed -- check A skipped; B falls back to a one-factor CFA on live CP\n")
  fit <- cfa('CP =~ CP1 + CP2 + CP3 + CP4', data = w, std.lv = TRUE)
...
cat("Not established: that S2's CP<k> labels and Table 2's CP<k> labels are one labelling (authors' own codes); English vs administered Chinese.\n")
