# verify_peters_2025_att_instr_strength.R
#
# CLAIM UNDER TEST (SKILL Step 5b), two axes:
#
#   AXIS 1 (item <-> item_text). Every IRW item code in
#   peters_2025_att_instr_strength is the lower-cased tail of exactly one
#   LimeSurvey question code `At(t)InEx<Tail>` in the Your COVID-19 Risk
...
#   D: if a Flip twin's anchors were mis-assigned relative to its partner, the
#      seven "Flip" twin pairs (same stem; anchors swapped in five, left
#      identical in two) would fit the rival relation better than the stated one.
#   E: if the anchor->code direction were globally reversed, items with an
#      unambiguous protective pole would sit on the wrong side of the midpoint.
#
...
#   - irw::irw_fetch() for check D only (a few MB; served from the local irw
#     cache when present, so normally no new export).
#
# Adapted 2026-09-24 (batch_385) from batch_133's verify_peters_2025_att_instr_eval.R,
# the paired evaluation-component table built from the same spreadsheet.

suppressMessages(library(irw))

TABLE <- "peters_2025_att_instr_strength"
...
CONSTRUCT <- "Instrumental attitude belief expectation"
...
# the build script's own DMQs_to_globally_eliminate vector, restricted to AttInEx
# (the full vector is GenEvDie, AttInEvFamilySafe, AttInEvChildFeel,
#  AttInEvChildReject, AttInExOthCrazy (listed twice), AttExEvEmbarr)
ELIM <- c("AttInExOthCrazy")
...
dmq$code <- tolower(sub("^At?tInEx", "", code))
...
## ---- CHECK D: Flip twins -- is each pair's swapped/identical status real? ---
# For each twin pair the spreadsheet either SWAPS the two anchors or leaves them
# IDENTICAL (a flip that was never applied at the source). Two rival predictions
# per pair: MIRROR, mean(flip) == (max+1) - mean(base); EQUAL, mean(flip) ==
# mean(base). The spreadsheet says which one should hold; the check passes a
# pair when the stated prediction fits at least twice as well as the rival.
# NOTE (corrects the batch_133 verify this was adapted from): the mirror
# relation is INVARIANT to a global reversal of the anchor->code direction, so
# this check does NOT pin the global direction. It checks that the anchors were
# assigned consistently WITHIN each pair. Check E pins the direction.
# (batch_385's first draft used a fixed 0.30 tolerance on the stated
# prediction; reasunifl missed it at 0.316 while the rival missed by ~1.9.
# Flip items fall systematically a little short of the mirror -- -0.12 to -0.32
# on the four swapped unipolar pairs -- which a fixed tolerance mistakes for a
# mapping failure.)
cat("\n-- CHECK D: flipped-twin pair structure on the live response data --\n")
TWINS <- list(c("uni", "unifl"), c("bi", "bifl"), c("traduni", "tradfl"),
              c("hybriduni", "hybridunifl"), c("implcomp", "implcompfl"),
              c("reasuni", "reasunifl"), c("reasbi", "reasbifl"))
d <- irw::irw_fetch(TABLE)
d$resp <- as.numeric(d$resp)
mu <- tapply(d$resp, d$item, mean, na.rm = TRUE)
nn <- tapply(d$resp, d$item, function(x) sum(!is.na(x)))
badD <- 0L
cat(sprintf("%-10s %-12s %-9s %7s %7s %8s %8s %8s %8s %s\n", "base", "flip",
            "anchors", "M_base", "M_flip", "pred", "|d|", "rival", "|d_riv|", "ok"))
for (p in TWINS) {
    a <- p[1]; b <- p[2]
    ...
    mirror <- (mx + 1) - mu[[a]]; equal <- mu[[a]]
    if (swapped) { pred <- mirror; rival <- equal } else if (identical_anchors) { pred <- equal; rival <- mirror } else { pred <- NA_real_; rival <- NA_real_ }
    ds <- abs(mu[[b]] - pred); dr <- abs(mu[[b]] - rival)
    ok <- !is.na(ds) && ds * 2 < dr
    if (!ok) badD <- badD + 1L
    cat(sprintf("%-10s %-12s %-9s %7.3f %7.3f %8.3f %8.3f %8.3f %8.3f %s\n", a, b,
                if (swapped) "swapped" else if (identical_anchors) "identical" else "other",
                mu[[a]], mu[[b]], pred, ds, rival, dr, if (ok) "ok" else "FAIL"))
}
cat(sprintf("(per-item n for the twins: %s)\n",
            paste(sprintf("%s=%d", unlist(TWINS),
                          as.integer(nn[unlist(TWINS)])), collapse = " ")))
cat(sprintf("twin pairs where the spreadsheet's stated relation fits >=2x better than the rival: %d/%d\n",
            length(TWINS) - badD, length(TWINS)))
if (badD > 0L) fail <- fail + 1L

## ---- CHECK E: global anchor->code direction, from content ----------------
# Items whose protective pole is unambiguous for someone who keeps their
# distance (the sample's modal stance: these are people answering a COVID
# precautions tool). For each, the side of the scale midpoint the live mean
# should fall on IF resp=1 is the bottom anchor as shipped. A global reversal
# would put every one of these on the wrong side.
cat("\n-- CHECK E: live mean falls on the protective pole of the SHIPPED anchors --\n")
EXPECT <- c(uni = "low", traduni = "low", hybriduni = "high", hybridunifl = "low",
            implcomp = "low", faminf = "low", friendsinf = "low", otherinf = "low",
            fined = "low", citizen = "high", familysafe = "high", house = "high",
            health = "high")
badE <- 0L
for (it in names(EXPECT)) {
    mx <- dmq$maxcode[dmq$code == it]; mid <- (mx + 1) / 2
    lo <- ship$option_text[ship$item == it & ship$resp == 1L][1]
    hi <- ship$option_text[ship$item == it & ship$resp == mx][1]
    side <- if (mu[[it]] < mid) "low" else "high"
    ok <- side == EXPECT[[it]]
    if (!ok) badE <- badE + 1L
    cat(sprintf("  %-12s M=%.2f (mid %.0f) expected %-4s  [1='%s' .. %d='%s'] %s\n",
                it, mu[[it]], mid, EXPECT[[it]], lo, mx, hi, if (ok) "ok" else "FAIL"))
}
cat(sprintf("items on the predicted side: %d/%d\n", length(EXPECT) - badE, length(EXPECT)))
if (badE > 0L) fail <- fail + 1L
