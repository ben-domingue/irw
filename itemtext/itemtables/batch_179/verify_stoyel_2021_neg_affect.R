# verify_stoyel_2021_neg_affect.R -- Step 5b mapping check (copied from references/verify_template.R)
#
# Code derivation is POSITIONAL: data/stoyel_2021_disordered_eating.py renames the NEGAF<k>.<wave>
# columns of the PLOS S1 workbook, sorted by integer k within each wave, to item_{i+1}. So the claim
# being shipped is item_k <-> header "NEGAF<k>.<w>: In the past week, I have felt... <adjective>".
#
...
TEXT <- c("Distressed", "Upset", "Guilty", "Scared", "Hostile",
          "Irritable", "Ashamed", "Nervous", "Jittery", "Afraid")   # shipped item_text for item_1..item_10
...
rebuilt <- list()
for (w in 1:3) {
    cols <- grep(sprintf("^NEGAF[0-9]+\\.%d\\s*:", w), names(raw), value = TRUE)
    k <- as.integer(sub(sprintf("^NEGAF([0-9]+)\\.%d.*", w), "\\1", cols))
    cols <- cols[order(k)]
