# verify_mexico_2023_quality_wellbeing.R -- batch_429, route 9 (response-frequency matching), per wave.
#
# Claim: IRW item p5_5_KK is INEGI ENCIG 2023 microdata column P5_5_KK, printed in the 2023
# questionnaire (p.9) as question 5.5 option KK (health service in the ISSSTE of this city),
# and p5_5a is P5_5A (question 5.5a, satisfaction). The 2021 rows (cov_year = 2021) come from
# ENCIG 2021 column P5_5_K (unpadded, K = 1..9; renamed onto P5_5_0K by RENAME_2021 in
# data/mexico_2023_quality.py) and P5_5_10, P5_5_11, P5_5A (same name, listed in ASKED_2021) --
# 2021 question 5.5 (p.8), the same ISSSTE question under the same number.
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P5_5A kept 1..6.
#
...
# column in the candidate pool of ALL section-V rating columns of its wave (every P5_* column
# except the 0-10 grades *B -- this includes the other health questions 5.4 IMSS, 5.6
# State/INSABI and 2023's 5.7 IMSS-Bienestar), namely its mapped one -- so a permutation of
# options, a shifted 2021 rename, or the wrong provider block (5.4/5.6/5.7 share 5.5's wording)
# fails. Corroboration: every 5.5 answerer reported ISSSTE use in 5.1 option 04
# (2023 P5_1_04 / 2021 P5_1_4 = 1), the printed filter "APLICA 5.5 Y 5.5a".
#
...
items <- c(sprintf("p5_5_%02d", 1:11), "p5_5a")
...
map23 <- setNames(c(sprintf("P5_5_%02d", 1:11), "P5_5A"), items)
map21 <- setNames(c(paste0("P5_5_", 1:11), "P5_5A"), items)     # RENAME_2021 (1..9) + same-name (10, 11, A)
pool <- function(m) grep("^P5_[0-9]+(_[0-9]+|A)$", names(m), value = TRUE)   # every section-V rating column (incl. the 5.1 screener; excl. 0-10 grades *B)
...
# Block corroboration: 5.5 is asked only of ISSSTE users (5.1 option 04).
filt <- function(m, flag) {
  ans <- rowSums(sapply(grep("^P5_5_", names(m), value = TRUE), function(v) m[[v]] %in% c("1", "2", "9"))) > 0
  c(answerers = sum(ans), with_flag = sum(ans & m[[flag]] == "1"), flag_users = sum(m[[flag]] == "1"))
}
f23c <- filt(w23, "P5_1_04"); f21c <- filt(w21, "P5_1_4")
cat(sprintf("\n5.5 answerers with 5.1 option 04 (ISSSTE) = 1: 2023 %d/%d (flag users %d), 2021 %d/%d (flag users %d)\n",
            f23c[2], f23c[1], f23c[3], f21c[2], f21c[1], f21c[3]))
ok_all <- ok_all && f23c[1] == f23c[2] && f21c[1] == f21c[2]

cat("Not established by this route: that column P5_5_KK is questionnaire option KK of 5.5 --\n",
