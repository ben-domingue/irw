# verify_okeke2025_decision_confidence_post.R -- batch_364, Step 5b.
#
# Claim: IRW item codes dc_post_1, dc_post_2, dc_post_3 are the source workbook's columns
# B16, B17, B18 ("Leverage AI Survey.xlsx", figshare 10.6084/m9.figshare.28768874, file
# 53579603), whose statements are printed against those same codes in "Leverage SC Survey
# Instrument.docx" (file 53579606), Section B table "Decision-Making Confidence (Post-use):".
#
# No IRW processing script for this table is in the repo, so the dc_post_k <- B(15+k)
...
# the "P" dropped; covariates checked to agree), then for each dc_post_k count
# per-respondent exact agreement with every one of the 32 Likert columns A1..A11, B1..B21.
# A correct mapping gives 200/200 on exactly one column, the claimed one; a swap of any two
# dc_post items, a shifted block (B15-B17, B17-B19), or confusion with the baseline
# Decision-Making Confidence block A6-A8 would put the 200 elsewhere.

suppressMessages(library(irw))

TABLE <- "okeke2025_decision_confidence_post"
CLAIM <- c(dc_post_1 = "B16", dc_post_2 = "B17", dc_post_3 = "B18")
...
for (it in names(CLAIM)) {
    ...
    cat(sprintf("%-10s %-7s %5d/%d %12s %14d\n", it, CLAIM[[it]], ag[[CLAIM[[it]]]], nrow(s), bo, max(oth)))
    cat("   agreement with B16/B17/B18:", paste(ag[c("B16", "B17", "B18")], collapse = "/"),
        "  with baseline A6/A7/A8:", paste(ag[c("A6", "A7", "A8")], collapse = "/"), "\n")
...
cat("\nEach dc_post item must agree 200/200 with its claimed column and fall well short on every other.\n")
