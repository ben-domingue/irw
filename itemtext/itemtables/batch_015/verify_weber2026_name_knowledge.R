## verify_weber2026_name_knowledge.R  --  batch_015, issue #1774
##
...
##
## Run from itemtext/:  Rscript itemtables/batch_015/verify_weber2026_name_knowledge.R
suppressMessages({library(jsonlite); library(irw)})
TBL <- "weber2026_name_knowledge"
CSV <- "itemtables/batch_015/weber2026_name_knowledge__items.csv"
...
cat("\n=== 2. item_text_translated vs the study's own English table ===\n")
tbad <- 0
for (code in names(src)) {
  want <- src[[code]]$translated
  got  <- unique(it$item_text_translated[it$item == code])
  if (length(got) != 1 || !identical(got, want)) tbad <- tbad + 1
}
cat(sprintf("  translated compared: %d | mismatches: %d\n", length(src), tbad))
if (tbad) fail <- c(fail, "item_text_translated mismatch")
