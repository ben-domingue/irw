# verify_friedman_2018_risks_discrimination.R -- batch_460, Step 5b.
#
# Claim: live IRW item codes ediscrim/eradeq/ewealth/hequal/hfeminin/hrevdis2 are the
# lowercased Qualtrics column names of Risk_Data_Base (Dataverse doi:10.7910/DVN/ZSJA25),
# and the codebook (Codebook.pdf pp.2-3) prints each statement under that exact column
...
# Route 6 (keying polarity): the three egalitarian-worded statements (e*) must correlate
# positively with each other and negatively with the three hierarchical ones (h*) --
...
TABLE <- "friedman_2018_risks_discrimination"
ITEMS <- c("ediscrim","eradeq","ewealth","hequal","hfeminin","hrevdis2")
...
for (it in ITEMS) {
  ...
  ok <- ok && same
  cat(sprintf("  %-9s src %s | live %s | unmapped %d | %s\n", it,
...
cat("\nRoute 6: polarity (egalitarian e* vs hierarchical h*), live data\n")
