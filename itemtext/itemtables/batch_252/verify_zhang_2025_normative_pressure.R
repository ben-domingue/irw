# verify_zhang_2025_normative_pressure.R -- Step 5b mapping check (batch_252).
#
# Claim: live item NP<k> is the item the paper's S2 File appendix prints as
# "NP<k>: <wording>". The appendix prefixes each item with the very code the S1
# Data column carries (explicit code labels), and data/zhang_2025_green_supply_chain.py
# melts S1 columns NP1..NP4 by name, so the code IS the source column name.
#
# Falsifiable checks run here:
#   A. live NP<k> equals S1 Data column NP<k> id-for-id, and no other NP column
#      reaches full agreement (the live code -> S1 column tie);
#   B. 5-factor CFA on S1 (CP, NP, EA, SE, GSCII, as in the paper) reproduces the
#      paper's Table 2 standardized loadings keyed to NP1..NP4 -- 0.807/0.854/
#      0.724/0.830 -- and every non-identity permutation of the four codes fits worse;
#   C. Cronbach's alpha of the live NP items vs Table 2 alpha 0.879.
# NOT established: that the S2 appendix's "NP<k>" labels and Table 2's "NP<k>"
...
PUB_LOAD <- c(NP1 = 0.807, NP2 = 0.854, NP3 = 0.724, NP4 = 0.830)   # Table 2 (t002)
PUB_ALPHA <- 0.879
TOL <- 0.02
...
if (ok) {
  ...
  obs <- setNames(ss$est.std[ss$lhs == "NP" & ss$op == "=~"], ss$rhs[ss$lhs == "NP" & ss$op == "=~"])[items]
} else {
  cat("S1 download failed -- check A skipped; B falls back to a one-factor CFA on live NP\n")
  fit <- cfa('NP =~ NP1 + NP2 + NP3 + NP4', data = w, std.lv = TRUE)
...
alt <- sapply(Filter(function(p) !identical(p, 1:4), perms(1:4)), function(p) max(abs(obs[p] - PUB_LOAD)))
cat(sprintf("largest |diff| identity: %.3f; best non-identity permutation: %.3f\n", worst, min(alt)))
if (worst > TOL || min(alt) <= worst) pass <- FALSE

X <- as.matrix(w[, items]); k <- 4
...
cat("Not established: that S2's NP<k> labels and Table 2's NP<k> labels are one labelling (authors' own codes); English vs administered Chinese.\n")
