# verify_mexico_2023_quality_streets.R -- batch_427, route 9 (response-frequency matching), per wave.
#
# Claim: IRW item p4_8_k is INEGI ENCIG microdata column P4_8_K, printed in both the 2023 and the
# 2021 questionnaire (Seccion IV) as question 4.8 option K (toll-free highways and roads,
# "las carreteras y caminos sin cuota de (ESTADO)"), and p4_8a is P4_8A (question 4.8a, satisfaction).
# NB: despite the table name, this is NOT question 4.7 ("las calles y avenidas de esta ciudad",
# streets and avenues), which is P4_7_* (the _roads sibling). The pool check below includes P4_7_*,
# so a streets/highways mix-up fails.
# Section IV was NOT renumbered between waves: RENAME_2021 in data/mexico_2023_quality.py has no
# p4_* entry and ASKED_2021 lists p4_8_1..4, p4_8a, so 2021 rows (cov_year = 2021) come from the
# same-named 2021 columns.
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P4_8A kept 1..6.
#
...
# neighbouring service block (4.7 streets and avenues, 4.6 police, ...) fails.
#
...
C <- ".cache/mexico_2023_quality_streets"
...
items <- c(paste0("p4_8_", 1:4), "p4_8a")
recode <- function(m, v) {
...
map23 <- setNames(c(paste0("P4_8_", 1:4), "P4_8A"), items)
...
cat("\nNot established by this route: that column P4_8_K is questionnaire option K of question 4.8 --\n",
