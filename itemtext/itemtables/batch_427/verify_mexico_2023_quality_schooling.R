# verify_mexico_2023_quality_schooling.R -- batch_427, route 9 (response-frequency matching), per wave,
# plus a routing check.
#
# Claim: IRW item p5_2_k is INEGI ENCIG microdata column P5_2_K, printed in both the 2023 and the
# 2021 questionnaire (Seccion V) as question 5.2 option K (public primary/secondary/upper-secondary
# education, "la educacion publica en primaria, secundaria o bachillerato"), and p5_2a is P5_2A
# (question 5.2a, satisfaction). Question 5.2 was NOT renumbered between waves (the 2023
# renumbering inserted 5.7 and 5.11, after it): RENAME_2021 in data/mexico_2023_quality.py has no
# p5_2* entry and ASKED_2021 lists p5_2_1..9, p5_2a, so 2021 rows (cov_year = 2021) come from the
# same-named 2021 columns.
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P5_2A kept 1..6.
#
# Predictions:
...
#      mapped one -- so a permutation of the nine yes/no codes, a flipped Si/No direction, or a
#      mix-up with a neighbouring block (5.3 university, 5.4 IMSS, ...) fails.
#  (b) routing: 5.1 option 01 (P5_1_01 in 2023, P5_1_1 in 2021; "o alguno(a) de sus hijos(as) fue
#      usuario(a) de educacion publica en primaria, secundaria o bachillerato?") routes Si to
#      "APLICA 5.2 Y 5.2a". So the persons with any answer on P5_2_1..9 must be exactly the
#      P5_1_01 = 1 set -- which ties the P5_2 block to the education screener option.
#
...
C <- ".cache/mexico_2023_quality_schooling"
...
items <- c(paste0("p5_2_", 1:9), "p5_2a")
recode <- function(m, v) {
...
map23 <- setNames(c(paste0("P5_2_", 1:9), "P5_2A"), items)
map21 <- map23                                                     # 5.2 not renumbered
...
# (b) routing check: answerers of the 5.2 block == Si set of 5.1 option 01
cat("\n== routing: 5.1 option 01 Si set vs 5.2 answerer set ==\n")
for (yr in c(2023, 2021)) {
  m <- if (yr == 2023) w23 else w21
  scr <- if (yr == 2023) "P5_1_01" else "P5_1_1"
  filled <- function(v) !is.na(m[[v]]) & trimws(m[[v]]) != ""        # fread reads blank cells as NA
  si <- m$ID_PER[filled(scr) & trimws(m[[scr]]) == "1"]
  blk <- paste0("P5_2_", 1:9)
  ans <- m$ID_PER[Reduce(`|`, lapply(blk, filled))]
  # which 5.1 screener option's Si set equals the 5.2 answerer set?
  ...
  ok_all <- ok_all && good
  cat(sprintf("%d: %s Si = %d; 5.2 answerers = %d; intersection = %d; screener options whose Si set equals it: %s%s\n",
              yr, scr, length(si), length(ans), length(intersect(si, ans)), paste(eq, collapse = ","),
              if (good) "" else "  <-- MISMATCH"))
}

cat("\nNot established by this route: that column P5_2_K is questionnaire option K of question 5.2 --\n",
