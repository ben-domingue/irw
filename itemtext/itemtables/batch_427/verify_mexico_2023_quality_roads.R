# verify_mexico_2023_quality_roads.R -- batch_427, route 9 (response-frequency matching), per wave.
#
# Claim: IRW item p4_7_k is INEGI ENCIG microdata column P4_7_K, printed in both the 2023 and the
# 2021 questionnaire (Seccion IV, p.6 of each PDF) as question 4.7 option K (streets and avenues,
# "las calles y avenidas de esta ciudad"), and p4_7a is P4_7A (question 4.7a, satisfaction).
# Section IV was NOT renumbered between waves: RENAME_2021 in data/mexico_2023_quality.py has no
# p4_* entry and ASKED_2021 lists p4_7_1..4, p4_7a, so 2021 rows (cov_year = 2021) come from the
# same-named 2021 columns.
#
# Resp coding per data/mexico_2023_quality.py: yes/no items 1 (Si) -> 0, 2 (No) -> 1;
# codes 9/98/99 -> missing; P4_7A kept 1..6.
#
...
# neighbouring service block (4.6 police, 4.8 toll-free highways and roads, ...) fails.
#
...
C <- ".cache/mexico_2023_quality_roads"
...
items <- c(paste0("p4_7_", 1:4), "p4_7a")
recode <- function(m, v) {
...
map23 <- setNames(c(paste0("P4_7_", 1:4), "P4_7A"), items)
...
cat("\nNot established by this route: that column P4_7_K is questionnaire option K of question 4.7 --\n",
