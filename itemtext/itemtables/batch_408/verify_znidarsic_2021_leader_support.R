# verify_znidarsic_2021_leader_support.R -- Step 5b, batch_408.
#
# Claim: item codes LS01..LS09 are POSITIONAL (data/znidarsic_2021_work_family_balance.py,
# df.iloc[:, 9:18] of PLOS S1 Data 10.1371/journal.pone.0245078.s001, sha256 fd8b260d...),
# and item_text is the trimmed column header at that position. The co-worker block
# (cols 18..26) asks the SAME nine behaviours, so the check must also show the codes do
# not come from that block. The script drops out-of-range values (one "41" in col 14),
# so the source counts below are over resp 1..5 only.
#
...
TABLE <- "znidarsic_2021_leader_support"
...
expected_col <- setNames(as.character(9:17), sprintf("LS%02d", 1:9))
...
cat("\nEach live item's 5-cell count vector matches exactly one of the 18 source columns (both\n",
    "blocks), and it is the leader column at the script's position; e.g. LS04 (17/43/71/71/45) and\n",
    "LS07 (17/35/54/70/71) share a count at resp 1 but differ elsewhere, and none equals its\n",
    "identically-worded co-worker column. This pins every item individually.\n",
