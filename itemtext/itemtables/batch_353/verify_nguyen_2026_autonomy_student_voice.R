# verify_nguyen_2026_autonomy_student_voice.R -- Step 5b mapping check (batch_353).
#
# Claim: item_text for V1..V3 is right because (a) the study's own
# Questionnaire.docx (Mendeley Data 10.17632/whsbpyxy6w, file 5e9f5e33-...)
# prints each item beside the very code the data use (table row "E STUDENT VOICE",
# V1..V3), and (b) the IRW item code IS the deposit .xlsx column
# header of the same name (data/nguyen_2026_learner_autonomy.py melts V1..V3
# by name; id = 1-based row index), so no positional step exists.
#
# What would break if item_text for two items were swapped:
#   check 1 -- shipped item_text for Vk must equal the questionnaire text
#              printed against code Vk, for every k (3/3);
#   check 2 -- the live IRW item Vk must agree cell-for-cell with xlsx column
#              Vk (row = id) and clearly less with any other V column (3x3).
# Not established: that the depositor filled xlsx column Vk from questionnaire
# item Vk -- that tie is the depositor's own code labelling, taken as
...
TABLE <- "nguyen_2026_autonomy_student_voice"
...
# ---- check 2: live (id, Vk) vs xlsx row id, column Sj -------------------
...
cat("\nNot established: that the depositor's xlsx column Vk holds answers to questionnaire\n",
    "item Vk -- that is the source's own code labelling, taken as authoritative. Nor does\n",
