# verify_shin2024_creactability_adaptability.R -- Step 5b mapping check (batch_172).
#
# Claim: Adaptability1 / Adaptability2 / Adaptability3 carry the paper's Table 2 Adaptability rows
# 1 / 2 / 3 ("He (she) adapted new situation well" / "endures stress well" / "learns well"),
...
#
# Bridge: live Adaptability counts must equal the figshare xlsx columns (source of the codes).

suppressMessages({ library(irw) })

TABLE <- "shin2024_creactability_adaptability"
SIBS  <- c("shin2024_creactability_quickness", "shin2024_creactability_creativity", TABLE)
...
cat("\nAll 6 assignments of Table 2 Adaptability rows 1-3 (Table 3 items 7-9) to the live codes:\n")
perms <- list(c(1,2,3), c(1,3,2), c(2,1,3), c(2,3,1), c(3,1,2), c(3,2,1))
fits <- 0
for (p in perms) {
  t2 <- tot; t2[7:9] <- tot[7:9][p]
  r <- max(abs(resid(lm(PUB_LOGIT ~ t2))))
  good <- r <= TOL; fits <- fits + good
  cat(sprintf("  rows 1,2,3 -> Adaptability%d,%d,%d : max |resid| %.4f  spearman %.2f %s\n",
...
cat("\n== Bridge: live Adaptability counts vs figshare xlsx columns ==\n")
...
if (got && requireNamespace("readxl", quietly = TRUE)) {
  ...
  for (it in paste0("Adaptability", 1:3)) {
...
cat("\nNOT established: the Korean wording coaches actually read (unpublished); that Table 3's item\n",
    "numbers follow Table 2's row order is the paper's only numbering and is what the fit tests.\n", sep = "")
