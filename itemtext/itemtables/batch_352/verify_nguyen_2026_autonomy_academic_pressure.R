# verify_nguyen_2026_autonomy_academic_pressure.R -- Step 5b mapping check (batch_352).
#
# Claim: item_text for AP1..AP3 is right because (a) the study's own
# Questionnaire.docx (Mendeley Data 10.17632/whsbpyxy6w, file 5e9f5e33-...)
# prints each item beside the very code the data use (table row "A ACADEMIC
# PRESSURE / ACADEMIC STRESS", AP1..AP3), and (b) the IRW item code IS the deposit .xlsx column
# header of the same name (data/nguyen_2026_learner_autonomy.py melts AP1..AP3
# by name; id = 1-based row index), so no positional step exists.
#
# What would break if item_text for two items were swapped:
#   check 1 -- shipped item_text for APk must equal the questionnaire text
#              printed against code APk, for every k (3/3);
#   check 2 -- the live IRW item APk must agree cell-for-cell with xlsx column
#              APk (row = id) and clearly less with any other AP column (3x3).
# Not established: that the depositor filled xlsx column APk from questionnaire
# item APk -- that tie is the depositor's own code labelling, taken as
...
TABLE <- "nguyen_2026_autonomy_academic_pressure"
here <- tryCatch(dirname(normalizePath(sys.frame(1)$ofile)), error = function(e) "itemtables/batch_352")
items_csv <- file.path(here, paste0(TABLE, "__items.csv"))
if (!file.exists(items_csv)) items_csv <- file.path("itemtables/batch_352", paste0(TABLE, "__items.csv"))
...
# ---- check 2: live (id, APk) vs xlsx row id, column APj -------------------
...
cat("\nNot established: that the depositor's xlsx column APk holds answers to questionnaire\n",
    "item APk -- that is the source's own code labelling, taken as authoritative. Nor does\n",
