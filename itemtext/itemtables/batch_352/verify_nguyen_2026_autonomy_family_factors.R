# verify_nguyen_2026_autonomy_family_factors.R -- Step 5b mapping check (batch_352).
#
# Claim: item_text for F1..F3 is right because (a) the study's own
# Questionnaire.docx (Mendeley Data 10.17632/whsbpyxy6w, file 5e9f5e33-...)
# prints each item beside the very code the data use (table row "C FAMILY
# FACTORS", F1..F3), and (b) the IRW item code IS the deposit .xlsx column
# header of the same name (data/nguyen_2026_learner_autonomy.py melts F1..F3
# by name; id = 1-based row index), so no positional step exists.
#
# What would break if item_text for two items were swapped:
#   check 1 -- shipped item_text for Fk must equal the questionnaire text
#              printed against code Fk, for every k (3/3);
#   check 2 -- the live IRW item Fk must agree cell-for-cell with xlsx column
#              Fk (row = id) and clearly less with any other F column (3x3).
# Not established: that the depositor filled xlsx column Fk from questionnaire
# item Fk -- that tie is the depositor's own code labelling, taken as
...
TABLE <- "nguyen_2026_autonomy_family_factors"
...
# ---- check 2: live (id, Fk) vs xlsx row id, column Fj -------------------
...
cat("\nNot established: that the depositor's xlsx column Fk holds answers to questionnaire\n",
    "item Fk -- that is the source's own code labelling, taken as authoritative. Nor does\n",
