# verify_nguyen_2026_autonomy_school_autonomy_support.R -- Step 5b mapping check (batch_352).
#
# Claim: item_text for S1..S3 is right because (a) the study's own
# Questionnaire.docx (Mendeley Data 10.17632/whsbpyxy6w, file 5e9f5e33-...)
# prints each item beside the very code the data use (table row "D SCHOOL AUTONOMY
# SUPPORT", S1..S3), and (b) the IRW item code IS the deposit .xlsx column
# header of the same name (data/nguyen_2026_learner_autonomy.py melts S1..S3
# by name; id = 1-based row index), so no positional step exists.
#
# What would break if item_text for two items were swapped:
#   check 1 -- shipped item_text for Sk must equal the questionnaire text
#              printed against code Sk, for every k (3/3);
#   check 2 -- the live IRW item Sk must agree cell-for-cell with xlsx column
#              Sk (row = id) and clearly less with any other S column (3x3).
# Not established: that the depositor filled xlsx column Sk from questionnaire
# item Sk -- that tie is the depositor's own code labelling, taken as
...
TABLE <- "nguyen_2026_autonomy_school_autonomy_support"
...
# ---- check 2: live (id, Sk) vs xlsx row id, column Sj -------------------
...
cat("\nNot established: that the depositor's xlsx column Sk holds answers to questionnaire\n",
    "item Sk -- that is the source's own code labelling, taken as authoritative. Nor does\n",
