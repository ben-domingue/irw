# verify_peters_2025_pbc_skill_prob.R  (batch_388)
#
# CLAIM UNDER TEST (Step 5b): each of the 14 live item codes carries the
# "Perceived subskill presence" stem of the SAME subskill, taken from that code's
# own row of the study's DMQs question-definition sheet (Google Sheet key
# 1B9TLM1ro2yxEmfzWo99T6w0XADLeeJVXPvnFoEc0-vc, worksheet 'en'), e.g.
#   remwalk -> "Typically, if you go out for a walk, are you able to remember ...",
# with resp=1 the bottom anchor "not able at all" and resp=5 "absolutely able".
#
# What would break if the mapping were wrong:
#   A. CODE RE-DERIVATION + TEXT: the sheet's `id` column holds the LimeSurvey
#      question code. Applying the project's own code shortening (dmqReplacements,
#      here Decline -> Decl) and the IRW script's rename
#      (data/peters_2025_covid19_risk_dcts.py: ^PbcSk(Pr|Im)(.+)$, Pr ->
#      pbc_skill_prob, tail lower-cased) must reproduce the live item set
#      one-to-one and onto, and each shipped stem/anchor must equal the row that
#      derives to that code. The DMQ_scales key must state uni codes 1..5 as
#      displayed (1)..(5), and the sheet's *_anchor_en must end "(1)" on the
#      bottom anchor and "(5)" on the top one (this fixes direction).
#   B. RAW -> LIVE CELL MATCH: for every respondent of three raw export files,
#      the value in raw column PbcSkPr<Subskill> must equal the live resp at
#      (that id, that item), cell for cell. The best rival PbcSkPr column and the
#      same-subskill PbcSkIm (importance) column are printed: a permuted tie, or a
#      Pr/Im component swap, collapses the matched-cell count.

suppressMessages(library(irw))
source(".claude/skills/irw-auto-itemtext/scripts/verify_cache.R")

TABLE  <- "peters_2025_pbc_skill_prob"
SHEET  <- paste0("https://docs.google.com/spreadsheets/d/",
                 "1B9TLM1ro2yxEmfzWo99T6w0XADLeeJVXPvnFoEc0-vc",
                 "/gviz/tq?tqx=out:csv&sheet=en")
SCALES <- paste0("https://docs.google.com/spreadsheets/d/",
                 "1GFFeIrwW9KVZxRCr4QrmQcVKDc5aSRu6VI4oWdQVkvw",
                 "/gviz/tq?tqx=out:csv&sheet=en")
RAW    <- "https://gitlab.com/a-bc/your-covid-19-risk-data/-/raw/master/data/"
CACHE  <- ".cache/peters_2025_pbc_skill_prob/"
FILES  <- c("YCR-dataPipeline--sid-100102--rids-1-141.csv",
            "YCR-dataPipeline--sid-100103--rids-1-1479.csv",
            "YCR-dataPipeline--sid-100105--rids-1-2450.csv")

args <- grep("^--file=", commandArgs(FALSE), value = TRUE)
items_csv <- if (length(args)) file.path(dirname(sub("^--file=", "", args[1])),
                 paste0(TABLE, "__items.csv")) else ""
if (!file.exists(items_csv))
    items_csv <- file.path("itemtables/batch_388", paste0(TABLE, "__items.csv"))
ship <- read.csv(items_csv, stringsAsFactors = FALSE)

bad <- 0L
shorten <- function(x) {
  reps <- c(generic = "gnrc", Decline = "Decl", Supermarket = "Sprmrkt",
            Neighbourhood = "Neighbrhd", NrmDeIde = "NrmDeId",
            Respons = "Rspns", Flip = "Fl")
  for (k in names(reps)) x <- gsub(k, reps[[k]], x)
  x
}

## ---- CHECK A ----
sheet <- read.csv(SHEET, stringsAsFactors = FALSE, check.names = FALSE)
pr <- sheet[grepl("^PbcSkPr", sheet$id), ]
pr$raw_col <- shorten(pr$id)
pr$derived <- tolower(sub("^PbcSkPr", "", pr$raw_col))
pr$im_col  <- sub("^PbcSkPr", "PbcSkIm", pr$raw_col)

s <- irw::irw_table_sets(TABLE, source = "core", per_item = TRUE)
live <- sort(s$items)
cat("== CHECK A: re-derive IRW codes from the sheet's own question codes ==\n")
miss_live  <- setdiff(live, pr$derived); miss_sheet <- setdiff(pr$derived, live)
cat(sprintf("sheet PbcSkPr rows: %d | live items: %d | live not derived: %d | derived not live: %d\n",
            nrow(pr), length(live), length(miss_live), length(miss_sheet)))
bij <- length(miss_live) == 0 && length(miss_sheet) == 0 &&
       length(unique(pr$derived)) == length(live)
bad <- bad + !bij
ok_txt <- 0L
for (i in order(pr$derived)) {
    cd <- pr$derived[i]; rows <- ship[ship$item == cd, ]
    st_ok <- identical(unique(rows$item_text), trimws(pr$subquestion_en[i]))
    lo <- rows$option_text[rows$resp == 1]; hi <- rows$option_text[rows$resp == 5]
    mid <- rows$option_text[rows$resp %in% 2:4]
    an_ok <- identical(lo, trimws(pr$bottom_anchor_bare[i])) &&
             identical(hi, trimws(pr$top_anchor_bare[i])) && all(is.na(mid) | mid == "")
    dir_ok <- grepl("\\(1\\)$", pr$bottom_anchor_en[i]) && grepl("\\(5\\)$", pr$top_anchor_en[i])
    ok <- st_ok && an_ok && dir_ok
    ok_txt <- ok_txt + ok
    cat(sprintf("%-20s -> %-12s stem=%-5s anchors=%-5s dir(1..5)=%-5s %s\n", pr$raw_col[i], cd,
                st_ok, an_ok, dir_ok, substr(pr$subquestion_en[i], 1, 70)))
}
cat(sprintf("shipped text equal to the deriving row: %d/%d\n", ok_txt, nrow(pr)))
bad <- bad + (ok_txt != nrow(pr))
sc <- read.csv(SCALES, stringsAsFactors = FALSE, check.names = FALSE)
uni <- sc[sc$scale_type == "uni", c("limesurvey_answer_code", "text_en")]
key_ok <- identical(as.integer(uni$limesurvey_answer_code), 1:5) &&
          identical(uni$text_en, sprintf("(%d)", 1:5))
cat(sprintf("DMQ_scales uni key: %s  -> %s\n",
            paste(uni$limesurvey_answer_code, uni$text_en, collapse = ", "), key_ok))
bad <- bad + !key_ok

## ---- CHECK B ----
d <- irw::irw_fetch(TABLE)
cat("\n== CHECK B: raw column PbcSkPr<X> vs live (id, item), cell for cell ==\n")
tot_eq <- 0L; tot_ne <- 0L; tot_live <- 0L; worst_rival_share <- 0
for (f in FILES) {
    path <- cached_source(paste0(CACHE, f), paste0(RAW, f))
    r <- read.csv(path, stringsAsFactors = FALSE, check.names = FALSE)
    sid <- sub(".*sid-(\\d+).*", "\\1", f)
    r$gid <- paste0(sid, "-", r[[1]])
    lv <- d[d$id %in% r$gid, ]
    cat(sprintf("%s: %d raw rows, %d live rows for these ids\n", f, nrow(r), nrow(lv)))
    for (i in order(pr$derived)) {
        cd <- pr$derived[i]; col <- pr$raw_col[i]
        if (!col %in% names(r)) { cat(sprintf("  %-12s raw column %s ABSENT\n", cd, col)); tot_ne <- tot_ne + 1L; next }
        v <- suppressWarnings(as.numeric(r[[col]]))
        keep <- !is.na(v) & v >= 1 & v <= 5
        raw_k <- setNames(v[keep], r$gid[keep])
        lvi <- lv[lv$item == cd, ]; live_k <- setNames(lvi$resp, lvi$id)
        same_ids <- setequal(names(raw_k), names(live_k))
        eq <- if (same_ids) sum(raw_k[names(live_k)] == live_k) else 0L
        ne <- length(live_k) - eq + if (same_ids) 0L else 1L
        rival <- sapply(setdiff(pr$raw_col, col), function(rc) {
            rv <- suppressWarnings(as.numeric(r[[rc]]))[match(names(live_k), r$gid)]
            sum(!is.na(rv) & rv == live_k)
        })
        imv <- suppressWarnings(as.numeric(r[[pr$im_col[i]]]))[match(names(live_k), r$gid)]
        im_eq <- sum(!is.na(imv) & imv == live_k)
        if (length(live_k) >= 10)
            worst_rival_share <- max(worst_rival_share, max(rival, im_eq) / length(live_k))
        tot_eq <- tot_eq + eq; tot_ne <- tot_ne + ne; tot_live <- tot_live + length(live_k)
        cat(sprintf("  %-12s n_live=%4d n_raw=%4d  equal %4d/%4d  mean %.3f/%.3f | best rival %s: %d | own PbcSkIm: %d\n",
                    cd, length(live_k), length(raw_k), eq, length(live_k),
                    if (length(raw_k)) mean(raw_k) else NA, if (length(live_k)) mean(live_k) else NA,
                    names(which.max(rival)), max(rival), im_eq))
    }
}
cat(sprintf("TOTAL cells equal %d of %d live, unequal/unmatched %d; worst rival share (items n>=10) %.2f\n",
            tot_eq, tot_live, tot_ne, worst_rival_share))
bad <- bad + (tot_ne != 0L) + (tot_eq == 0L) + (worst_rival_share >= 0.9)

cat("\nWHAT THIS DOES NOT ESTABLISH: B ties each IRW code to its raw LimeSurvey column\n",
    "on the respondents of 3 of the 50 export files only (sids 100102, 100103, 100105);\n",
    "the column->wording tie rests on the DMQs sheet's own `id` column (A), and no check\n",
    "here can detect an error inside that sheet. Direction rests on the published\n",
    "answer-code key and the sheet's '(1)'/'(5)' anchor suffixes, not on a data-side test.\n",
    "Midpoints 2-4 are unlabelled in the source and ship blank; the non-English\n",
    "administered wordings are not shipped or tested.\n", sep = "")

cat(if (bad == 0L) "VERDICT: PASS\n" else "VERDICT: FAIL\n")
