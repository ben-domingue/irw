# Step 5b verification for peters_2025_si_current (batch_388).
#
# CLAIM UNDER TEST
#   The 6 IRW items are the 6 checkbox options of the Your COVID-19 Risk tool's
#   LimeSurvey multiple-choice question `siCurrent` ("Are you doing any of the
#   following?"). data/peters_2025_covid19_risk_precautions.py takes each raw column
#   "siCurrent.<code>." and lower-cases <code> (all six codes are already lower case),
...
#   C. Keying polarity (route 6): `home` ("Mostly staying at home") must correlate
#      negatively with all five leaving-the-house items, and those five must
#      correlate positively with one another.
#
# WHAT IT DOES NOT ESTABLISH
#   - The raw-column -> wording tie is a code-label match (ls_answer_code = subquestion
#     title = raw column infix); response data cannot test wording for the five
#     leaving-the-house items, which C does not separate from one another.
...
here <- tryCatch(dirname(sys.frame(1)$ofile), error = function(e) "itemtables/batch_388")
...
r <- r[r$ls_question_code == "siCurrent", ]
...
qrow <- which(cls == "Q" & nm == "siCurrent" & lg == "en")
...
long <- do.call(rbind, lapply(FILES, function(fn) {
  ...
  names(df)[1] <- "orig_id"
  cols <- grep("^siCurrent\\.[A-Za-z]+\\.$", names(df), value = TRUE)
  do.call(rbind, lapply(cols, function(cn) {
    code <- sub("^siCurrent\\.([A-Za-z]+)\\.$", "\\1", cn)
    est  <- paste0("siCurrent", toupper(substr(code, 1, 1)), substring(code, 2), "Est")
...
out5 <- setdiff(colnames(cm), "home")
hm <- cm["home", out5]
pairs <- cm[out5, out5][upper.tri(cm[out5, out5])]
cat(sprintf("\nC. cor(home, other) over %d respondents: %s\n", nrow(w),
            paste(sprintf("%s %.2f", out5, hm), collapse = ", ")))
cat(sprintf("   %d leaving-the-house item pairs: min %.2f, max %.2f\n", length(pairs), min(pairs), max(pairs)))
c_ok <- all(hm < 0) && all(pairs > 0)

ok <- a_ok == NITEM && stem_ok && b_ok && c_ok
cat(sprintf("\nA %s  B %s  C %s\n", a_ok == NITEM && stem_ok, b_ok, c_ok))
cat("Not established: wording for the five leaving-the-house items rests on the code-label\n",
