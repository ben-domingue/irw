...
#
# CHECK A (composite): SDQ_Pro == round(mean(SDQ_Pro1..5)) for every respondent.
...
m <- rowMeans(w[, its])
a_ok <- all(w$SDQ_Pro == round(m))
cat("CHECK A -- SDQ_Pro == round(mean(SDQ_Pro1..5)) for all",
    nrow(w), "respondents:", a_ok, "\n")
