...
def _dedupe_adjacent(marks, v):
    ...
    cell reads "10^-^4". The second mark is redundant only when it starts
    exactly where the first one ended -- i.e. it continues the same run.
    """
    out, end = [], {}
    for pos, kind, ln in sorted(marks):
        e = end.get(kind)
        # A mark is redundant when it starts at or before the END of the run
        # already marked -- it continues or overlaps it. Two shapes occur:
        # "10^-4" arrives as "-" then "4", abutting; and Ce4+ arrives as a
        # two-character span "4+" PLUS a one-character span "+", overlapping.
        # Testing exact abutment catches the first and misses the second
        # ("Ce^4^+"); testing "within 6 characters", as this once did, catches
        # both but also eats the separate subscripts of "(SO_4)_3" and
        # "H_2SO_4". The end cursor is what distinguishes continuation from
        # proximity, and it must advance even when nothing is emitted, or a
        # three-span run keeps its third span.
        if e is not None and pos <= e:
            end[kind] = max(e, pos + ln)
            continue
        out.append((pos, kind)); end[kind] = pos + ln
    return out
    ...
    for i in range(d.page_count):
        for blk in d[i].get_text("dict")["blocks"]:
            for ln in blk.get("lines", []):
                ...
                if len(sp) < 2:
                    continue
                # ONE OUTSIZED GLYPH MUST NOT DEFINE "BODY SIZE". A 12pt
                # reaction arrow on a 9.75pt line made max() report 12, so the
                # script test (size >= body*0.85 -> >= 10.2) classified the
                # ORDINARY TEXT as script, and the line came out "H_2^O",
                # "C^6H5O-". Weight by characters, so the size that sets most
                # of the line wins and a lone arrow cannot outvote it.
                sizes = collections.Counter()
                for s_ in sp:
                    t_ = s_["text"].strip()
                    if t_:
                        sizes[round(s_["size"], 2)] += len(t_)
                if not sizes:
                    continue
                body = sizes.most_common(1)[0][0]
                # Measure from the BODY baseline, not from whatever sits
                # lowest on the line -- which is normally the subscripts, so
                # every offset was taken against a reference already pushed
                # down by the thing being measured.
                bl = [s_["bbox"][3] for s_ in sp
                      if round(s_["size"], 2) == body and s_["text"].strip()]
                base = max(bl) if bl else max(s_["bbox"][3] for s_ in sp)
                run = ""
                for s in sp:
                    t = s["text"]
                    ...
                    if left.strip() and t.strip():
                        # Record whether CTX cut this context mid-word; the
                        # start-boundary guard in main() must not fire on a
                        # context that never claimed to start at a boundary.
                        out.append((left, t, kind, len(run) > CTX))
                    run += t
    return out
    ...
    for p in sorted(glob.glob(os.path.join(a.items_dir, "*__items.csv"))):
        ...
        for r in rows:
            for col in ("item_text", "option_text"):
                ...
                marks = []
                for left, txt, kind, trunc in spans:
                    core = txt.strip()
                    ...
                    # ... but only when the context really does start at a
                    # boundary. CTX=24 truncation put "S2 (s) + 15 O2 (g) + 2 H"
                    # in front of the subscript of H2O, whose first character
                    # lands inside "FeS"; the guard then rejected a valid,
                    # 24-character-anchored span. A 24-char context is an
                    # anchor on its own -- the guard is for one-char ones.
                    guard = (r"(?<![A-Za-z\u00c0-\u00ff0-9])"
                             if left[:1].isalnum() and not trunc else "")
                    pat = re.compile(guard + _loose(left) + "(" + _loose(core) + ")")
                    hits = list(pat.finditer(v))
                    if not hits and re.match(r"[A-E][\t ]+\S", left):
                        # The PRINTED line starts with the option letter
                        # ("A\t 7,5 × 10"), but the parser has already
                        # stripped that letter from option_text, so the
                        # context can never match and the exponent is lost:
                        # 2013 CN 23920 shipped a stem reading "6 × 10^23"
                        # beside options reading "1,5 × 1025". Retry once
                        # with exactly the prefix the parser removes.
                        alt = re.sub(r"^[A-E][\t ]+", "", left)
                        if alt.strip():
                            hits = list(re.finditer(
                                _loose(alt) + "(" + _loose(core) + ")", v))
                    ...
                    if len(hits) > 1:
                        ambiguous[kind] += 1
                        continue
                    marks.append((hits[0].start(1), kind, len(core)))
                seen_pos = set()
