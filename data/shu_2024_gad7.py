...
# adolescents." PeerJ. CC BY 4.0. N=1190. Raw GAD-7 items (text-coded
# 4-point frequency scale, recoded 0-3 in category order); GADtotal is
...
# mapping. PHQ-9 items from the same file are shipped separately as
# shu_2024_phq9.py per datastandard.md's "one file per scale" rule.
...
def convert():
    ...
    out_path = OUT_DIR / "shu_2024_gad7.csv"
    long.to_csv(out_path, index=False)
    print(f"shu_2024_gad7: rows={len(long)} ids={long['id'].nunique()} "
