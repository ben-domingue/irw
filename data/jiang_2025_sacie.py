...
# efficacy. PLoS ONE.
#
# S1 Data (XLSX, N=240 Chinese pre-service teachers). Scale of
# Sentiments, Attitudes, and Concerns about Inclusive Education
# (SACIE-R, Forlin et al. 2011), 15 items across 3 dimensions
# (sentiments 1-5, attitudes 1-5, concerns 1-5), 6-point Likert (1-6).
# Companion scales from the same survey/sample: jiang_2025_teaching_motivation
# (4 items), jiang_2025_inclusive_efficacy (18 items), jiang_2025_empathy
# (7 items).
...
ITEM_COLS = (
    [f"sentiments{i}" for i in range(1, 6)]
    + [f"attitudes{i}" for i in range(1, 6)]
    + [f"concerns{i}" for i in range(1, 6)]
)
