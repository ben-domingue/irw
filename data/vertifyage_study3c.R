...
data <- read_sav("Study 3c_IM_Data.sav")
data <- data %>% 
  mutate(across(everything(), ~`attr<-`(.x, "label", NULL)))

data <- data[!is.na(data$Q4.4), ]

data <- data %>%
  select(ResponseId, Q33correct:Q59correct,Q4correct:Q47correct, Q4.4) %>%
...
saveRDS(data_1, "vertifyage_study3c.RData")         
