##https://www.kaggle.com/datasets/julesking/tla-lab-persuade-dataset?select=persuade_train_srctexts.csv

## wget -O persuade_train_srctexts.csv "https://storage.googleapis.com/kagglesdsdata/datasets/5585591/10630493/persuade_train_srctexts.csv?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=gcp-kaggle-com%40kaggle-161607.iam.gserviceaccount.com%2F20260312%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20260312T172345Z&X-Goog-Expires=259200&X-Goog-SignedHeaders=host&X-Goog-Signature="

x<-read.csv("persuade_train_srctexts.csv")
L<-split(x,x$essay_id_comp)

f<-function(x) {
    text<-paste(x$discourse_text,collapse=' ')
    z<-data.frame(id=unique(x$essay_id_comp),item=unique(x$assignment),source_text=unique(x$source_text_1),text=text)
    for (nm in c("gender", "grade_level", "ell_status", "race_ethnicity", "economically_disadvantaged", "student_disability_status")) z[[paste("cov_",nm,sep='')]]<-unique(x[[nm]])
    z
}
df<-data.frame(do.call("rbind",lapply(L,f)))

df$text<-gsub("\"","'",df$text)
df$text<-gsub("\n"," ",df$text)
df$source_text<-gsub("\"","'",df$source_text)
df$source_text<-gsub("\n"," ",df$source_text)
df$item<-gsub("\"","'",df$item)
df$item<-gsub("\n"," ",df$item)

write.table(df,file="persuade_learningagency.csv",row.names=FALSE,sep="|")

