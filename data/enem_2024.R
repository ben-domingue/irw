...
# 2024 microdata ships as two row-aligned files instead of a single MICRODADOS
# CSV: PARTICIPANTES_2024.csv holds NU_INSCRICAO, RESULTADOS_2024.csv holds the
# CO_PROVA_* booklet codes and TX_RESPOSTAS_* response strings. The two files are
# in the same row order (equal row counts), so we column-bind them.
participantes <- vroom('DADOS/PARTICIPANTES_2024.csv',
                       delim=';',
                       col_select = list(id = NU_INSCRICAO))

resultados <- vroom('DADOS/RESULTADOS_2024.csv',
                    delim=';',
                    col_select = list(starts_with('CO_PROVA'),
                                      starts_with('TX')))

stopifnot(nrow(participantes) == nrow(resultados))

microdata <- bind_cols(participantes, resultados) |>
  drop_na()
...
items <- vroom('DADOS/ITENS_PROVA_2024.csv',
               delim=';',
...
save(df, file='enem_2024_1mil_lc.Rdata')
...
save(df, file='enem_2024_1mil_ch.Rdata')
...
save(df, file='enem_2024_1mil_cn.Rdata')
...
save(df, file='enem_2024_1mil_mt.Rdata')
