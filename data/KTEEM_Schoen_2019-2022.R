...
df1 <- df1 %>%
  # `DataCollectionWave` is Spring19-Spring22 and `PublicID` persists across
  # them -- 832 of 1,130 ids recur -- so it is a wave, not a sample. Calling it
  # `group` meant nothing downstream would accept it as the reason a person
  # answers the same item five times, and 49,357 repeats read as unexplained
  # (irw#1842 block F). No data changes; with the column named `wave` the
  # excess is zero.
  dplyr::rename(id = PublicID, wave = DataCollectionWave) %>%
