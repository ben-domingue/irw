"""Perceived Stress Scale, Beck Depression Inventory and STAI form X in a Czech general-population sample.

DOI: 10.17632/kmkhx3tzjx
Source: https://data.mendeley.com/datasets/kmkhx3tzjx
License: CC BY 4.0
Contributor (deposit record): Figalova, Charvat

1,725 respondents across three instruments, shipped as four tables:
PSS-14 (0-4, the seven reverse-keyed items keep their 'r' suffix as
deposited), BDI-21 (0-3), and STAI form X split into its two standard
20-item halves -- X1 (state) and X2 (trait) -- which are separate
administrations on the same 1-4 scale.

This deposit was triaged 'recoverable_format': the file is
semicolon-delimited, so a default read produced a single 1725x1 column.
Reading with sep=';' recovers all 79 columns.
"""
...
TABLE_PREFIX = "figalova_2021_"
BLOCKS = {'pss': 'PSS_\\d+r?', 'bdi': 'BDI_\\d+', 'stai_state': 'STAI_X1_\\d+', 'stai_trait': 'STAI_X2_\\d+'}            # table suffix -> regex matching its item columns
COVS = {'SEX': 'cov_sex', 'AGE': 'cov_age', 'EDUCATION': 'cov_education'}
DROP = ['UID']
DROP_REASON = 'row identifier, replaced by a sequential id'
SCALE = {'pss': (0, 4), 'bdi': (0, 3), 'stai_state': (1, 4), 'stai_trait': (1, 4)}              # table suffix -> (min, max), or None to infer
QC_WAIVERS = {}
