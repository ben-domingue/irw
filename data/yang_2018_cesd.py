...
# (E1-E20, 1-4 scale) and a parallel 20-item anxiety scale (E21-E40,
# 1-4 scale). This script produces the depression table; see
# yang_2018_anxiety.py for the companion anxiety table. All other columns
# in the file are demographic/risk-behavior covariates or precomputed
# subscale sums (depressionS, anxietyS, TotalC2, D6total, SocialCapital,
# etc.) and are excluded per the composite-exclusion rule.
...
ITEM_COLS = [f"E{i}" for i in range(1, 21)]
...
def convert():
    ...
    # Documented scale is 1-4; drop anything outside that range as a
    # data-entry artifact.
    ...
    out_path = os.path.join(OUT_DIR, "yang_2018_cesd.csv")
    long.to_csv(out_path, index=False)

    print(f"yang_2018_cesd: rows={len(long)} ids={long['id'].nunique()} "
