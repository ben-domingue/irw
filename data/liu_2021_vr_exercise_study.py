...
OUTPUT_DIR = "liu_2021_vr_exercise_study"
OUTPUT_BREQ = os.path.join(OUTPUT_DIR, "liu_2021_vrexp_breq.csv")
OUTPUT_BRUMS = os.path.join(OUTPUT_DIR, "liu_2021_vrexp_brums.csv")
OUTPUT_DEPRESSION = os.path.join(OUTPUT_DIR, "liu_2021_vrexp_depression.csv")
OUTPUT_GENERAL_OTHER = os.path.join(OUTPUT_DIR, "liu_2021_vrexp_general_other.csv")
...
def convert_vr_exercise_to_irw():
    ...
    item_cols = [c for c in raw_item_cols if not _is_excluded_item_column(c)]
    ...
    out_abs = os.path.join(base, OUTPUT_DIR)
    ...
    readiness_path = os.path.join(base, OUTPUT_DIR, "liu_2021_vrexp_readiness.csv")
    if os.path.isfile(readiness_path):
        os.remove(readiness_path)

    written: list[pd.DataFrame] = []
    for rel_fname, icols in outputs:
        part = _melt_items(df, id_col, cov_candidates, pre_cols, icols)
        path = os.path.join(base, rel_fname)
        if part is None or part.empty:
            print(f"{rel_fname}: skipped (no columns or empty)")
            continue
        part.to_csv(path, index=False)
        written.append(part)
        print(
            f"{rel_fname}: rows={len(part)}, ids={part['id'].nunique()}, "
            f"items={part['item'].nunique()}, resp_range=({part['resp'].min()}, {part['resp'].max()})"
        )
