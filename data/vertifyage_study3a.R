...
data <- read_sav("Study 3a_AA_Data.sav")
data <- data %>% 
  mutate(across(everything(), ~`attr<-`(.x, "label", NULL)))
questions <- c("Q4", "Q6","Q99", "Q11", "Q13", "Q23", "Q33", "Q40", "Q41", "Q45", "Q47", "Q49", "Q59", "Q61", "Q63", "Q65", "Q67", "Q69", "Q75")
for(q in questions) {
  correct_col <- paste0(q, "correct")
  data[[correct_col]][is.na(data[[q]])] <- NA
}
data <- data %>%
  select(ResponseId, Q4correct:Q99correct, Q57) %>%
...
saveRDS(data_1, "vertifyage_study3a.RData")       
