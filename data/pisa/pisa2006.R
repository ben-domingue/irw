...
filepath="/Users/radhika/Google Drive Stanford/IRW/PISA/2006/"
setwd(filepath)
library(haven)

# importing data in a readable format
dic_student = parse.SAScii(sas_ri = 'PISA2006_SAS_cognitive_item.sas')
student <- read_fwf(file = 'INT_Cogn06_S_Dec07.txt', col_positions = fwf_widths(dic_student$width), progress = T)
...
df= student |>
  select(COUNTRY,CNT, SUBNATIO,STIDSTD, SCHOOLID,BOOKID,
         M033Q01:S527QSC) |>
  pivot_longer(
    cols= M033Q01:S527QSC,
    names_to= "item",
    values_to= "resp"
  )

unique(df$resp) #7= NA, 8= not reached, 9=missing
n_distinct(df$item)
...
###### Add student weights
dic_student = parse.SAScii(sas_ri = 'PISA2006_SAS_student.sas')
student <- read_fwf(file = 'INT_Stu06_Dec07.txt', col_positions = fwf_widths(dic_student$width), progress = T)
colnames(student) <- dic_student$varname
#glimpse(student)
...
student = student |> select(contains("WT"), contains("FAC"), "COUNTRY", "CNT", "STIDSTD","SCHOOLID","ST01Q01", "ST04Q01") |>
  rename(id= STIDSTD, 
         gender= ST04Q01,
         grade = ST01Q01) |>
...
for (sub in c("math", "read", "science")) {
  ...
  name = paste0("pisa2006_", sub, ".Rdata")
