...
How incrementality works:
  ...
  - A term run for the first time here has no prior row, so it falls back
    to --default-lookback-days (default 90) before today.
...
def last_run_date(term: str, log_path: str = LOG_PATH) -> str | None:
    """Most recent date this monthly script itself ran `term`, or None if
    it never has. Only considers rows whose output_file this script wrote
    (see OUT_PREFIX) -- ignores rows logged by other parts of the pipeline
    that happen to reuse the same term text (e.g. PLOS batches)."""
    best = None
    try:
