...
11. **Run the validator** — `irw-validate path/to/table.csv` (install with `pip install irw-validate`; `python3 -m irw_validate.cli` from a checkout). Exit code `1` means something blocks upload; `red_up` runs the same `upload` profile on every Redivis upload, so a table that fails here will be refused there. Its warnings are heuristics — read them, but a documented reason can justify one.
